...
test("resolveSearchConfig keeps default model when efforts is not set", () => {
  assert.deepEqual(resolveSearchConfig("grok-4.20-fast"), {
    model: "grok-4.20-fast",
  });
});

test("resolveSearchConfig maps efforts=low to grok-4.20-reasoning", () => {
  assert.deepEqual(resolveSearchConfig("grok-4.20-fast", "low"), {
    model: "grok-4.20-reasoning",
  });
});

test("resolveSearchConfig maps efforts=medium to multi-agent low effort", () => {
  assert.deepEqual(resolveSearchConfig("grok-4.20-fast", "medium"), {
    model: "grok-4.20-multi-agent",
    reasoningEffort: "low",
  });
});

test("resolveSearchConfig maps efforts=high to multi-agent high effort", () => {
  assert.deepEqual(resolveSearchConfig("grok-4.20-fast", "high"), {
    model: "grok-4.20-multi-agent",
...
test("buildResponsesRequestBody injects reasoning preset for efforts=medium", () => {
  const body = buildResponsesRequestBody("OpenAI 官网", "grok-4.20-fast", "medium");
